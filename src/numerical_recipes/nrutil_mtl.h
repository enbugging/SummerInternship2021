...
template <class T> class NRVec {
  ...
  public:
    ...
    NRVec(const NRVec<T> &rhs) : myvec(rhs.myref.size()), myref(myvec) { copy(rhs.myref, myref); }
...
template <class T> class NRMat {
  ...
  public:
    ...
    NRMat(const NRMat &rhs) : mymat(rhs.myref.nrows(), rhs.myref.ncols()), myref(mymat) { copy(rhs.myref, myref); }
    inline NRMat &operator=(const NRMat &rhs) {
        if (myref.nrows() != rhs.myref.nrows() && myref.ncols() != rhs.myref.ncols()) {
